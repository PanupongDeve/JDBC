...
public class TableDisplay extends JFrame 
{
  ...
  {   
    ...
    //   executeQuery(conn, "select distinct EditionNumber from Titles");

    //print First Name
    //   executeQuery(conn, 
    //     "SELECT FirstName FROM Authors WHERE AuthorID IN (SELECT AuthorID FROM AuthorISBN WHERE ISBN IN (SELECT ISBN FROM Titles WHERE Title LIKE 'C*'))");

    //printf Name have ++
    executeQuery(conn, "SELECT Title FROM Titles WHERE Title LIKE '*++*'");    
    //   executeQuery(conn, "SELECT * from Authors");
    } 
